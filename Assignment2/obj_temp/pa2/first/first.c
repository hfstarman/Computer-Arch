...
void printResult(FILE *outFile, tNode* root, char* word, int height);
...
int main(int argc, char** argv){
	...
	while(fscanf(mapFile, "%s %s", dict, data) != EOF){
		...
		char word[200] = {0};
		printResult(outFile, root, word, 0);
		fclose(outFile);		
...
void printResult(FILE *outFile, tNode* root, char* word, int height){
	...
	if(root->isWord == 1){
		...
		fprintf(outFile, "%s %d %d\n", word, root->occurrences, root->superwords);
	}
	...
	for(i = 0; i < 26; i++){
		...
		if(root->children[i] != NULL){
			word[height] = ('a'+i);
			printResult(outFile, root->children[i], word, height+1);
		}
